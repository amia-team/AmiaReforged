...
/// Follows the same patterns as <see cref="LoreApiService"/> and <see cref="ResourceNodeApiService"/>.
/// </summary>
public class InteractionApiService
{
    ...
    private async Task<T?> PostAsync<T>(string url, object? body, bool rawJson = false) where T : class
    {
        var (baseUri, apiKey) = await ResolveEndpointAsync();
        var http = _httpClientFactory.CreateClient("WorldEngine");
