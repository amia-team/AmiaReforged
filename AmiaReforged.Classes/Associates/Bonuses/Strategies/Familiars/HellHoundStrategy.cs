...
public class HellHoundStrategy : IFamiliarBonusStrategy
{
    ...
    public void Apply(NwCreature owner, NwCreature associate)
    {
        ...
        if (associate.Level < 15) return;

        SpecialAbility elementalShield = new(NwSpell.FromSpellType(Spell.ElementalShield)!, 15);
        associate.AddSpecialAbility(elementalShield);
    }
}
