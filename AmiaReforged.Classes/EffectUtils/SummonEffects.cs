...
public static class SummonEffects
{
        ...
        float minLoc, float maxLoc, float minDelay, float maxDelay, int summonVfx = 0, float summonVfxFloat = 1)
    {
        for (int i = 1; i <= summonCount; i++)
        {
            ...
            float newDelay = delay + 0.09f;
            string newTag = summonResRef + IntToString(i) + CreaturePlugin.GetOriginalName(caster, FALSE);
            ...
            DestroyObject(GetObjectByTag(newTag), summonDuration);
        }
    }

    public static bool IsPactSummon(uint target)
    {
        return GetResRef(target) is "wlkfiend" or "wlkfey" or "wlkslaad" or
