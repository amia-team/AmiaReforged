...
public class AbilityRestrictionsHandler
{
    ...
    public AbilityRestrictionsHandler()
    {
        ...
        Log.Info(message: "Monk Ability Restrictions Handler initialized.");
    }
    ...
    private static void PreventBodyOrSpiritTechnique(OnUseFeat eventData)
    {
        ...
        if (hasFocusWithoutUnarmed)
            player.SendServerMessage(
                $"Having equipped a focus without being unarmed has prevented your {eventData.Feat.Name}.");
    }
    ...
    private static void PreventStaticBonuses(OnEffectApply eventData)
    {
        ...
        if (hasArmor)
            player.SendServerMessage(message: "Equipping this armor has disabled your monk abilities.");
        if (hasShield)
    ...
    private static void PreventTechniqueInNoCastingArea(OnUseFeat eventData)
    {
        ...
        if (eventData.Creature.GetClassInfo(ClassType.Monk) is null) return;
        if (eventData.Creature.Area?.GetObjectVariable<LocalVariableInt>(name: "NoCasting").Value is 0) return;
    ...
    public static bool PreventMartialTechnique(OnEffectApply eventData)
    {
        if (eventData.Object is not NwCreature monk) return false;
        ...
        bool isTechniquePrevented = false;

        if (hasArmor || hasShield || hasFocusWithoutUnarmed)
        {
            eventData.PreventApply = true;
            isTechniquePrevented = true;
        }
        ...
        if (hasFocusWithoutUnarmed)
            player.SendServerMessage(
                $"Having equipped a focus without being unarmed has prevented your {techniqueName}.");
