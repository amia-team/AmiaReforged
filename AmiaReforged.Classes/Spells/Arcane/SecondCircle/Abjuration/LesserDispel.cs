...
namespace AmiaReforged.Classes.Spells.Arcane.SecondCircle.Abjuration;

/// <summary>
/// Lesser Dispel - An abjuration spell that attempts to strip magical effects.
///
/// Targeted: attempts to dispel all effects on the target.
/// Area: attempts to dispel the most powerful effect on each creature.
///
/// Dispel check: 1d20 + caster level (max +5) vs DC 11 + spell effect's caster level.
...
public class LesserDispel(DispelService dispelService) : ISpell
{
    private const int BaseCasterLevelCap = 5;
    private const int MaxCasterLevel = 30;

    public bool CheckedSpellResistance { get; set; }
    public bool ResistedSpell { get; set; }
    public string ImpactScript => "NW_S0_LsDispel";

    public void SetSpellResisted(bool result) => ResistedSpell = result;

    /// <summary>Lesser Dispel bypasses spell resistance entirely.</summary>
    ...
    public void OnSpellImpact(SpellEvents.OnSpellCast eventData)
    {
        ...
        Effect breachVfx = Effect.VisualEffect(VfxType.ImpHeadSonic);
        Effect impactVfx = Effect.VisualEffect(VfxType.FnfLosNormal20);
        ...
        Effect breachVfx, NwSpell spell)
    {
        switch (GetObjectType(currentTarget))
        {
            ...
            case OBJECT_TYPE_PLACEABLE:
                SignalEvent(currentTarget, EventSpellCastAt(caster.ObjectId, SPELL_LESSER_DISPEL));
                break;
