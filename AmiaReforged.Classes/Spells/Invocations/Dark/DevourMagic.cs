...
public class DevourMagic
{
    public void CastDevourMagic(uint nwnObjectId)
    {
        ...
        if (NwEffects.GetHasEffectType(EFFECT_TYPE_PETRIFY, target) == TRUE ||
            GetLocalInt(target, sVarName: "X1_L_IMMUNE_TO_DISPEL") == 10) return;
        ...
        // * Casting as AoE
        //-----------------
        const int objectTypes = OBJECT_TYPE_CREATURE | OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE |
        ...
        while (GetIsObjectValid(currentTarget) == TRUE)
        {
            ...
            if (GetObjectType(currentTarget) == OBJECT_TYPE_PLACEABLE ||
                GetObjectType(currentTarget) == OBJECT_TYPE_DOOR)
            {
            ...
            {
                if (NwEffects.GetHasEffectType(EFFECT_TYPE_PETRIFY, target) == TRUE ||
                    GetLocalInt(target, sVarName: "X1_L_IMMUNE_TO_DISPEL") == 10)
                {
                    currentTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, location, FALSE, objectTypes);
                    continue;
                }
