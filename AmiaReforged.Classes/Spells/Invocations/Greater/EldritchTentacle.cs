...
public static class EldritchTentacle
{
    public static void StrikeTargetWithTentacle(uint target, uint caster)
    {
        ...
        if (targetSize == CREATURE_SIZE_TINY) targetOpposeCheck += -8;

        if (targetSize == CREATURE_SIZE_SMALL) targetOpposeCheck += -4;

        if (targetSize == CREATURE_SIZE_LARGE) targetOpposeCheck += 4;

        if (targetSize == CREATURE_SIZE_HUGE) targetOpposeCheck += 8;
