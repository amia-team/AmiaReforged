...
public class WrithingDark
{
    public void CastWrithingDark(uint nwnObjectId)
    {
        ...
        if (GetLocalInt(GetArea(nwnObjectId), darknessProhibited) == TRUE)
            SendMessageToPC(nwnObjectId, szMessage: "The spell fizzles in this location!");

        IntPtr aoe = NwEffects.LinkEffectList(new List<IntPtr>
        {
            EffectAreaOfEffect(AOE_PER_DARKNESS, sOnEnterScript: "****", sHeartbeatScript: "****",
                sOnExitScript: "****"),
        ...
        NwEffects.RemoveAoeWithTag(location, nwnObjectId, aoeTag: "VFX_PER_WLK_DARK", RADIUS_SIZE_COLOSSAL);
        NwEffects.RemoveAoeWithTag(location, nwnObjectId, aoeTag: "VFX_PER_DARKNESS", RADIUS_SIZE_COLOSSAL);
