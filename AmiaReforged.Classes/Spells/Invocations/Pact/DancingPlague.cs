...
public class DancingPlague
{
    ...
    public void CastDancingPlague(uint nwnObjectId)
    {
        ...
        if (GetIsImmune(target, IMMUNITY_TYPE_DISEASE) == TRUE || GetLocalInt(target, "has_danced") == TRUE)
        {
            return;
        }
    ...
    private void DanceParty(uint caster, float effectDuration, IntPtr location)
    {
        ...
        while (GetIsObjectValid(currentTarget) == TRUE)
        {
            ...
            {
                if (GetIsImmune(currentTarget, IMMUNITY_TYPE_DISEASE) == TRUE || GetLocalInt(currentTarget, "has_danced") == TRUE)
                {
                ...
                if (passedFortSave)
                {
                    ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_FORTITUDE_SAVING_THROW_USE), currentTarget);
                    currentTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_MEDIUM, location);
                    continue;
                }
                if (!passedFortSave)
