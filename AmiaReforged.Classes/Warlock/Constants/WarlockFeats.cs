using Anvil.API;

namespace AmiaReforged.Classes.Warlock.Constants;

public static class WarlockFeats
{
    public const Feat OtherworldlyResilience = (Feat)1297;
    public const Feat EldritchMaster = (Feat)1298;
    public const Feat EpicEldritchBlast1 = (Feat)1300;
    public const Feat ArmoredCaster = (Feat)1307;
    public const Feat WarlockDamageReduction = (Feat)1308;
    public const Feat EnergyResistanceAcid = (Feat)1309;
    public const Feat EnergyResistanceCold = (Feat)1310;
    public const Feat EnergyResistanceElectrical = (Feat)1311;
    public const Feat EnergyResistanceFire = (Feat)1312;
    public const Feat EnergyResistanceSonic = (Feat)1313;
    public const Feat AberrantPact = (Feat)1314;
    public const Feat CelestialPact = (Feat)1315;
    public const Feat FeyPact = (Feat)1316;
    public const Feat FiendPact = (Feat)1317;
    public const Feat ElementalPact = (Feat)1318;
    public const Feat SlaadPact = (Feat)1319;
}
