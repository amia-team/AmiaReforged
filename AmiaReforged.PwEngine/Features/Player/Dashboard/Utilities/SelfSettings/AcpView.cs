using AmiaReforged.PwEngine.Features.WindowingSystem.Scry;
using Anvil.API;

namespace AmiaReforged.PwEngine.Features.Player.Dashboard.Utilities.SelfSettings;

public sealed class AcpView : IScryView
{
    public NuiBind<int> StyleSelected { get; } = new("style_selected");
    public NuiBind<List<NuiComboEntry>> StyleOptions { get; } = new("style_options");

    public NuiLayout RootLayout()
    {
        NuiColumn root = new()
        {
            Children = new List<NuiElement>
            {
                new NuiRow
                {
                    Width = 0f,
                    Height = 0f,
                    Children = new List<NuiElement>(),
                    DrawList = new List<NuiDrawListItem> { new NuiDrawListImage("ui_bg", new NuiRect(-10f, -10f, 390f, 290f)) }
                },
                new NuiSpacer { Height = 10f },
                new NuiLabel("Phenotype Changer (WIP)")
                {
                    Height = 20f,
                    ForegroundColor = new Color(30, 20, 12),
                    HorizontalAlign = NuiHAlign.Center
                },
                new NuiSpacer { Height = 5f },
                new NuiLabel("Select a phenotype to change your animations.")
                {
                    Height = 35f,
                    HorizontalAlign = NuiHAlign.Center,
                    ForegroundColor = new Color(60, 30, 30)
                },
                new NuiSpacer { Height = 10f },
                new NuiRow
                {
                    Children = new List<NuiElement>
                    {
                        new NuiSpacer { Width = 10f },
                        new NuiCombo
                        {
                            Entries = StyleOptions,
                            Selected = StyleSelected,
                            Width = 320f,
                            Height = 25f
                        },
                        new NuiSpacer { Width = 10f }
                    }
                },
                new NuiSpacer { Height = 20f },
                new NuiRow
                {
                    Children = new List<NuiElement>
                    {
                        new NuiSpacer(),
                        new NuiButton("Set Phenotype")
                        {
                            Id = "btn_set",
                            Width = 120f,
                            Height = 35f
                        },
                        new NuiSpacer { Width = 10f },
                        new NuiButton("Cancel")
                        {
                            Id = "btn_cancel",
                            Width = 100f,
                            Height = 35f
                        },
                        new NuiSpacer()
                    }
                },
                new NuiSpacer { Height = 10f }
            }
        };
        return root;
    }
}
