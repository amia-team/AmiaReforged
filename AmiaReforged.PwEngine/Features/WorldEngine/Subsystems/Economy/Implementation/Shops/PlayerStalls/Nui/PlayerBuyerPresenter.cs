...
public sealed class PlayerBuyerPresenter : ScryPresenter<PlayerBuyerView>, IAutoCloseOnMove
{
    ...
    private void HandleSelectProduct(int rowIndex)
    {
        if (rowIndex < 0 || rowIndex >= _productRows.Count)
        {
            Log.Warn("Invalid row index {RowIndex} for product selection (count: {Count})", rowIndex,
                _productRows.Count);
            return;
        }
        ...
        Token().SetBindValue(View.QuantityMaxLabel,
            string.Format(CultureInfo.InvariantCulture, "/ {0} max", product.QuantityAvailable));
        Token().SetBindValue(View.TotalCostLabel,
            string.Format(CultureInfo.InvariantCulture, "Total: {0}", FormatPrice(product.Price)));
    ...
    private async Task ExamineSelectedProductAsync()
    {
        ...
        try
        {
            ...
            await item.ActionJumpToLocation(loginCreatureLocation);
    ...
    private void InitializeFilterOptions(IReadOnlyList<PlayerStallProductView> products)
    {
        ...
        var uniqueTypes = products
            ...
            .Select(p => (Type: p.BaseItemType!.Value,
                Name: p.ItemTypeName ??
                      BaseItemTypeNameResolver.GetDisplayName(p.BaseItemType) ?? $"Type {p.BaseItemType}"))
            .DistinctBy(t => t.Type)
            .OrderBy(t => t.Name);
