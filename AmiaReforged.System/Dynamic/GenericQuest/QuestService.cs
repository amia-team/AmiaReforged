...
public class QuestService
{
    ...
    private static void OnScriptCalled(CallInfo callInfo)
    {
        ...
        LocalVariableString legacyQuestName = questGiver.GetObjectVariable<LocalVariableString>(name: "questname");
        LocalVariableString questNames = questGiver.GetObjectVariable<LocalVariableString>(name: "quest names");
        if (legacyQuestName.HasNothing && questNames.HasNothing)
            playerCharacter.LoginPlayer!.SendServerMessage
                (message: "DEBUG: No quest names found in quest NPC's local variables.");

        LocalVariableString legacySpeechWithQuest =
            questGiver.GetObjectVariable<LocalVariableString>(name: "speechwithquest");
        LocalVariableString messagesWithQuest =
            questGiver.GetObjectVariable<LocalVariableString>(name: "messages with quest taken");
        if (legacySpeechWithQuest.HasNothing && messagesWithQuest.HasNothing)
            playerCharacter.LoginPlayer!.SendServerMessage
                (message: "DEBUG: No messages with quest taken found in quest NPC's local variables.");

        LocalVariableString legacySpeechQuestDone =
            questGiver.GetObjectVariable<LocalVariableString>(name: "speechquestdone");
        LocalVariableString messagesWithQuestDone =
            questGiver.GetObjectVariable<LocalVariableString>(name: "messages with quest completed");
        if (legacySpeechQuestDone.HasNothing && messagesWithQuestDone.HasNothing)
            playerCharacter.LoginPlayer!.SendServerMessage
                (message: "DEBUG: No messages with quest completed found in quest NPC's local variables.");


        LocalVariableString questItemTag = questGiver.GetObjectVariable<LocalVariableString>(name: "questitem");
        LocalVariableString questItemTags = questGiver.GetObjectVariable<LocalVariableString>(name: "quest item tags");
        if (questItemTag.HasNothing && questItemTags.HasNothing)
            playerCharacter.LoginPlayer!.SendServerMessage
                (message: "DEBUG: No quest item tags found in quest NPC's local variables.");
