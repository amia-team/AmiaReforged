...
public class DayNightEncounterSpawner : IEncounterSpawner
{
    ...
    private static bool DoSpawnsVary() => NWScript.GetLocalInt(_area, sVarName: "spawns_vary") == 1;
    ...
    private static IEnumerable<string> GetResRefsForPrefix(string prefix)
    {
        ...
        if (numberOfLocalVars == 0) return new List<string>();
    ...
    {
        int level = NWScript.GetLevelByPosition(1, creature) + NWScript.GetLevelByPosition(2, creature) +
                    NWScript.GetLevelByPosition(3, creature);
        ...
        NWScript.SetLocalInt(creature, sVarName: "CustDropPercent", 50);
    ...
    private static void ApplyCageyStatus(uint creature)
    {
        int level = NWScript.GetLevelByPosition(1, creature) + NWScript.GetLevelByPosition(2, creature) +
                    NWScript.GetLevelByPosition(3, creature);
        if (level < 3) level = 3;
        IntPtr eAC = NWScript.EffectACIncrease(level / 3);
